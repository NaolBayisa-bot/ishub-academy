...
import type { IUser } from '@ishub/shared';
...
export default function AdminDashboard() {
  ...
  const statusBadge = (status: string) => {
    ...
    return (
      <span className={`inline-flex px-2.5 py-0.5 rounded-tech text-xs font-mono font-medium ${colors[status] || 'bg-slate-100 text-slate-800'}`}>
        {status}
      </span>
    );
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1 font-mono text-sm">~/admin</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-8">
        {[
          { label: 'Total Students', value: students.length },
          { label: 'Pending', value: students.filter((s) => s.status === 'PENDING').length },
          { label: 'Active', value: students.filter((s) => s.status === 'ACTIVE').length },
          { label: 'Rejected', value: students.filter((s) => s.status === 'REJECTED').length },
        ].map((stat) => (
          <Card key={stat.label} className="text-center">
            <p className="text-2xl font-bold text-slate-900 dark:text-white font-mono">{stat.value}</p>
      ...
      <Card>
        <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Student Registrations</h3>

        {isLoading ? (
          <Spinner className="py-12" />
        ) : students.length === 0 ? (
          <p className="text-gray-400 dark:text-gray-500 text-center py-8">No student registrations yet.</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200 dark:border-gray-700">
                  ...
                  <th className="text-left py-3 px-2 font-medium text-gray-500 dark:text-gray-400">Department</th>
              ...
              <tbody>
                {students.map((student) => (
                  <tr key={student.id} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                    ...
                    <td className="py-3 px-2">{statusBadge(student.status)}</td>
