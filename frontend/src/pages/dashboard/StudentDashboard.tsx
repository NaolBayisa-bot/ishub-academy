...
export default function StudentDashboard() {
  ...
  const totalLessons = trainings.reduce((sum: number, t: any) => sum + (t?.totalLessons ?? 0), 0);
  const totalCompleted = trainings.reduce((sum: number, t: any) => sum + (t?.completedLessons ?? 0), 0);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">
          <span className="text-primary-500 font-mono">{'>'} </span>Welcome, {user?.firstName} {user?.lastName}!
        </h1>
      ...
      <Card className="mb-8">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-3">Overall Progress</h3>
        {loading ? (
          ...
          <div>
            <div className="flex items-center gap-3">
              <div className="flex-1 h-2 bg-slate-200 dark:bg-tech-border rounded-full">
                <div className="h-2 bg-green-500 rounded-full transition-all" style={{ width: `${overallPercent}%` }} />
              </div>
              <span className="text-xs text-slate-500">{overallPercent}%</span>
            </div>
            <p className="text-xs text-slate-500 mt-2">{totalCompleted} of {totalLessons} lessons completed</p>
          </div>
        )}
      </Card>
      ...
      <Card className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Your Trainings</h3>
          <Button size="sm" onClick={() => window.location.href = '/trainings'}>Browse Trainings</Button>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          <div>
            <p className="text-gray-500 dark:text-gray-400">First Name</p>
            <p className="font-medium dark:text-gray-200">{user?.firstName}</p>
          </div>
          <div>
