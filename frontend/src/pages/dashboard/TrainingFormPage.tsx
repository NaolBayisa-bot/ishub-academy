...
export default function TrainingFormPage() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      setCategories(data);
      if (data.length && !isEdit) setForm((f) => ({ ...f, categoryId: String(data[0].id) }));
    } catch (err: any) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-6">{isEdit ? 'Edit Training' : 'Create Training'}</h1>
      ...
      <Card className="p-6">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <Button variant="secondary" type="button" onClick={() => navigate('/admin/trainings')}>Cancel</Button>
          </div>
        </form>
      </Card>
