...
export default function TrainingAccessGrantsPanel({ trainingId }: Props) {
  ...
  const handleSearch = async () => {
    ...
    setSearching(true);
    try {
      const { data } = await api.get('/users/search', { params: { email: searchEmail } });
  ...
  const handleGrant = async (userId: number) => {
    setGranting(userId);
    try {
      const { data } = await api.post(`/trainings/${trainingId}/access-grants`, { userId });
      setGrants((prev) => [{
        id: data.id,
        userId: data.user.id,
  ...
  return (
    <Card className="p-6 mt-6">
      <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-4">Manual Access Grants</h3>
      ...
      <div className="flex gap-2 mb-4">
        ...
        />
        <Button size="sm" onClick={handleSearch} isLoading={searching}>Search</Button>
      </div>
        ...
        <div className="mb-4 border border-slate-200 dark:border-tech-border rounded-md divide-y divide-slate-200 dark:divide-tech-border">
          {searchResults.map((student) => (
            <div key={student.id} className="flex items-center justify-between px-3 py-2">
              <span className="text-sm text-slate-700 dark:text-slate-300">{student.firstName} {student.lastName} ({student.email})</span>
              <Button size="sm" onClick={() => handleGrant(student.id)} isLoading={granting === student.id}>Grant</Button>
            </div>
          ))}
        </div>
      )}
      ...
      {loading ? (
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">No students have been granted access yet.</p>
      ) : (
        <ul className="divide-y divide-slate-200 dark:divide-tech-border">
          {grants.map((grant) => (
            <li key={grant.id} className="flex items-center justify-between py-2">
              <div>
                <p className="text-sm font-medium text-slate-900 dark:text-white">{grant.user.firstName} {grant.user.lastName}</p>
                <p className="text-xs text-slate-500 dark:text-slate-400">{grant.user.email}</p>
              </div>
              <Button variant="danger" size="sm" onClick={() => handleRevoke(grant.id)}>Revoke</Button>
            </li>
          ))}
        </ul>
      )}
